...
import { ArrowUpRight, ArrowDownRight, Plus, Trash2, X, Mic, MicOff, Search } from "lucide-react";
...
export default function TransactionsPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    await addTransaction({ type: fType, amount: Number(fAmt), category: fCat, date: fDate, notes: fNotes, merchantName: fMerch, createdAt: new Date().toISOString() });
    ...
    setShowForm(false); setFAmt(""); setFNotes(""); setFMerch(""); await load();
  }
  ...
  return (
    <div className="p-5 space-y-4">
      ...
      <div className="animate-fade-in-up delay-1 space-y-2.5">
      ...
      {showForm && (
        <form onSubmit={submit} className="glass-card rounded-2xl p-4 space-y-3.5 animate-scale-in">
          ...
          <button type="submit" className="btn-primary w-full py-3 text-sm">Simpan Transaksi</button>
        </form>
      )}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-4">
          {Object.entries(grouped).map(([label, items], gi) => (
            <div key={label} className="animate-fade-in-up" style={{ animationDelay: `${gi*0.05}s` }}>
              <p className="text-xs font-bold text-foreground/30 uppercase tracking-wider mb-2 px-1">{label}</p>
              <div className="space-y-2">
                {items.map((tx, i) => (
                  <div key={tx.id} className="glass-card flex items-center justify-between rounded-xl p-3.5 group animate-fade-in-up" style={{ animationDelay: `${(gi*5+i)*0.03}s` }}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => del(tx.id)} aria-label="Hapus transaksi" className="opacity-0 group-hover:opacity-100 p-1.5 text-foreground/20 hover:text-red-500 rounded-lg hover:bg-red-500/10 transition-all"><Trash2 size={14}/></button>
                    </div>
                  </div>
                ))}
              </div>
